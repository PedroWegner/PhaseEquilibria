...
if __name__ == '__main__':
    ...
    mixture = Mixture([dioxide, water], k_ij=k_ij, l_ij=0.0)
    ...
    T_co2 =  np.linspace(220, 302.4, 300)
    P_water_0 = 60e5
    P_co2_0 = 5e5
    P_water = []
    T_water_c = []
    ...
    mixture_test = Mixture(components=[dioxide], k_ij=0.0, l_ij=0.0)
    ...
    plt.plot(T_water_c, P_water, linewidth=1.25, color='black')
    plt.plot(T_co2_c, P_co2, linewidth=1.25, color='black')
    plt.xlabel('T / ºC')
    plt.ylabel('P / bar')
    plt.ylim(top=1200, bottom=30)
    plt.xlim(left=-50, right=425)
    plt.show()
